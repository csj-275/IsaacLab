...
- 在__init__.py中导入
### 2.场景配置
- 参考`scripts/tutorials/02_scene`中的脚本创建场景，测试机器人描述


### 3.环境配置
- 执行`./isaaclab -n` 创建内部manager_based任务
- 在创建的任务下的env_cfg中修改场景配置类，将前面通过测试的类复制过去即可
- 参考`scripts/tutorials/03_envs`中的脚本创建环境，主要包括ActionsCfg, ObservationsCfg, EnentCfg的配置，测试各个类的编写是否正确
