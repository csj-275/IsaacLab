...
class CuroboPlannerCfg:
    ...
    def piper_grab_config(cls) -> "CuroboPlannerCfg":
        """Create configuration for Piper robot grab task.

        .. note::
            This configuration requires a URDF file for the Piper robot.
            Set ``PIPER_URDF_PATH`` to the absolute path of your Piper URDF file.
            If the URDF is not available, the SkillGen data generation will fall
            back to a warning and the user should provide the URDF before use.

        Returns:
            CuroboPlannerCfg: Configuration for Piper robot
        """
        # ==============================================================================
        # IMPORTANT: Before using SkillGen with Piper, you need to provide a URDF file.
        #
        # 1. Export URDF from your Piper robot description:
        #    - If you have the Piper ROS2 package, the URDF is typically at:
        #      `piper_description/urdf/piper.urdf`
        #    - If you only have the USD file, you can export it from Isaac Sim
        #      using the "File -> Export Selected As..." menu with URDF format.
        #
        # 2. Set the path below to point to your URDF file:
        # ==============================================================================
        import os

        # Try to find Piper URDF from common locations
        urdf_path = os.environ.get(
            "PIPER_URDF_PATH",
            "",  # Default: not set, will check common locations
        )

        # If PIPER_URDF_PATH env var is not set, try common locations
        if not urdf_path:
            common_paths = [
                "/home/csj/Desktop/IsaacLab/usd/piper.urdf",
                "/home/csj/Desktop/IsaacLab/source/isaaclab_assets/isaaclab_assets/robots/piper/piper.urdf",
                "/etc/piper/piper.urdf",
            ]
        ...
        if not urdf_path or not os.path.exists(urdf_path):
            print(
                "WARNING: Piper URDF not found. SkillGen cuRobo planning will not be available for Piper.\n"
                "  To use SkillGen, set the PIPER_URDF_PATH environment variable to your Piper URDF file.\n"
                "  Example: export PIPER_URDF_PATH=/path/to/piper.urdf\n"
                "  Falling back to Franka config as placeholder."
            )
            # Return a minimal config - planning will fail gracefully
            return cls(
                robot_name="piper",
                gripper_joint_names=["joint7", "joint8"],
                gripper_open_positions={"joint7": 0.1, "joint8": -0.1},
                gripper_closed_positions={"joint7": -0.1, "joint8": 0.1},
                ...
                motion_noise_scale=0.02,
                static_objects=["box", "table"],
                world_ignore_substrings=["/World/defaultGroundPlane", "/curobo"],
            )

        # URDF found - create full config
        robot_cfg_file = cls._create_temp_robot_yaml(
            "franka.yml",  # Use franka.yml as template (will be overridden by Piper URDF)
            urdf_path,
        )
        ...
        return cls(
            ...
            gripper_open_positions={"joint7": 0.1, "joint8": -0.1},
            gripper_closed_positions={"joint7": -0.1, "joint8": 0.1},
            hand_link_names=["link7", "link8", "link6"],
            attached_object_link_name="attached_object",
            static_objects=["box", "table"],
            grasp_gripper_open_val=0.04,
            approach_distance=0.05,
            retreat_distance=0.05,
            max_planning_attempts=15,
            time_dilation_factor=0.6,
            enable_finetune_trajopt=True,
            n_repeat=None,
            motion_step_size=None,
            ...
            world_ignore_substrings=["/World/defaultGroundPlane", "/curobo"],
            get_world_config=lambda: cls._get_world_config_with_table_adjustment_static(),
        )

    @staticmethod
    def _get_world_config_with_table_adjustment_static() -> WorldConfig:
        """Static version of world config with table adjustment for Piper.
        ...
        return WorldConfig(cuboid=world_cfg_table.cuboid, mesh=world_cfg_mesh.mesh)
    ...
    def from_task_name(cls, task_name: str) -> "CuroboPlannerCfg":
        ...
        if "piper-grab" in task_lower or "piper_grab" in task_lower:
